...
// Modelo de datos: representa un snack del inventario
class ProductoInfo
{
    ...
    // Constructor vacio: necesario para parsear lineas del CSV
    public ProductoInfo() { }

    // Constructor con parametros: permite crear un objeto ya inicializado
...
class Program
{
    ...
    static void Main()
    {
        ...
        do
        {
            ...
            // Si el usuario no ingresa un numero valido, vuelve al menu
    ...
    // CONTROL 1: Agregar un snack manualmente al CSV
    static void AgregarProducto()
    {
        ...
        // El ID nuevo se basa en el maximo existente para evitar duplicados al eliminar y agregar
    ...
    // CONTROL 2: Mostrar todos los snacks en formato tabla
    static void MostrarProductos()
    {
        ...
        Console.WriteLine();
        Console.WriteLine("{0,-5}{1,-25}{2,-10}{3,-8}{4,-10}{5}", "ID", "PRODUCTO", "PESO", "STOCK", "PRECIO", "ESTADO");
        Console.WriteLine(new string('-', 75));

        foreach (var p in lista)
        {
            Console.WriteLine("{0,-5}{1,-25}{2,-10}{3,-8}S/.{4,-8:F2}{5}",
                p.Id, p.Nombre, p.Peso, p.Stock, p.Precio,
                p.Disponible ? "Disponible" : "No Disponible");
        }
    }

    /* CONTROL 3: Buscar snack por nombre
       Repite la pregunta hasta que el usuario encuentre algo o presione Enter para cancelar */
    static void BuscarProducto()
    {
        ...
        while (true)
        {
            Console.Write("Ingrese el nombre del snack (Enter para cancelar): ");
            string buscar = Console.ReadLine()?.Trim().ToLower() ?? "";

            // Enter sin texto cancela y vuelve al menu
            if (buscar == "") return;

            var resultados = lista.Where(x => x.Nombre.ToLower().Contains(buscar)).ToList();
    ...
    // CONTROL 4: Contar cuantos snacks hay en el CSV
    static void ContarProductos()
    {
        Console.WriteLine($"Total de snacks: {LeerTodos().Count}");
    }

    // CONTROL 5: Exportar datos del CSV a archivo binario (.dat)
    static void GuardarBinario()
    {
        ...
        using (BinaryWriter bw = new BinaryWriter(File.Open(archivoBinario, FileMode.Create)))
        {
            ...
            foreach (var p in lista)
            {
                // El orden de escritura debe coincidir exactamente con el orden de lectura en LeerBinario
                bw.Write(p.Id);
                bw.Write(p.Nombre);
                bw.Write(p.Peso);
                bw.Write(p.Stock);
    ...
    // CONTROL 6: Leer y mostrar datos desde el archivo binario (.dat)
    static void LeerBinario()
    {
        ...
        using (BinaryReader br = new BinaryReader(File.Open(archivoBinario, FileMode.Open)))
        {
            ...
            {
                // El orden de lectura debe coincidir exactamente con el orden de escritura en GuardarBinario
                int id = br.ReadInt32();
    ...
    /* CONTROL 8: Modificar un snack existente por ID
       Repite la pregunta hasta que el ID sea valido o el usuario cancele con Enter */
    static void ModificarProducto()
    {
        ...
        while (true)
        {
            Console.Write("Ingrese el ID (Enter para cancelar): ");
            ...
            producto = lista.FirstOrDefault(p => p.Id == id);
        ...
        if (!string.IsNullOrWhiteSpace(peso)) producto.Peso = peso;
        ...
        if (double.TryParse(Console.ReadLine(), out double precio)) producto.Precio = precio;
    ...
    /* CONTROL 9: Eliminar un snack por ID
       Repite la pregunta hasta que el ID sea valido o el usuario cancele con Enter */
    static void EliminarProducto()
    {
        ...
        while (true)
        {
            Console.Write("Ingrese el ID del producto (Enter para cancelar): ");
            string input = Console.ReadLine() ?? "";
    ...
    /* CONTROL 10: Cambiar estado de un snack (toggle Disponible <-> No Disponible)
       Repite la pregunta hasta que el ID sea valido o el usuario cancele con Enter */
    static void CambiarEstadoProducto()
    {
        ...
        // Toggle: invierte el valor booleano (true -> false, false -> true)
    ...
    /* Convierte una linea del CSV en un objeto ProductoInfo
       Devuelve null si la linea esta mal formada o tiene menos campos de los esperados */
    static ProductoInfo? ParsearLinea(string linea)
    {
        ...
        // Si el campo Disponible no existe o no es parseable, se asume true por defecto
        producto.Disponible = partes.Length >= 6 && bool.TryParse(partes[5], out bool estado) ? estado : true;

        return producto;
    }

    // Lee el CSV completo y devuelve una lista en memoria con todos los productos
    ...
    /* Reescribe el CSV completo a partir de una lista en memoria
       Todas las funciones que modifican datos usan esta en vez de escribir el archivo por su cuenta */
    ...
    // CONTROL 7: Precargar la lista oficial del equipo con 53 productos reales
